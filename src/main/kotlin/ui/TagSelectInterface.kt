...
data class TagData(val id: Int, val name: String)

class TagSelectContext(val namespaceId: Int, var filter: TodoFilter) : ScrollContext()
...
object TagSelectInterface : ScrollInterface<TagSelectContext, TagData>(
    ...
    override fun contentProvider(id: Int, context: TagSelectContext): TagData? {
        return TagManager.byNamespace(context.namespaceId).getOrNull(id)?.let {
            TagData(it[TagManager.Tags.id].value, it[TagManager.Tags.name])
        }
    }

    override fun contentDisplay(data: TagData, context: TagSelectContext): InterfaceInfo<TagSelectContext>.() -> ItemStack = {
        val state = context.filter.stateOf(data.id)
        val (material, stateLabel, loreColor) = when (state) {
    ...
    override fun contentClick(data: TagData, context: TagSelectContext): ClickInfo<TagSelectContext>.() -> Unit = {
        context.filter = context.filter.toggle(data.id)
        TagSelectInterface.openInventory(click.player, context)
    }

    override fun getOtherItems(): List<InterfaceItem<TagSelectContext>> = listOf(
        item()
            .atSlot(8)
