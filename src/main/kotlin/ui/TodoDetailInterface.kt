...
class TodoDetailContext(val todoId: Int) : Context()
...
object TodoDetailInterface : RoosterInterface<TodoDetailContext>(
    ...
    override fun getInterfaceItems(): List<InterfaceItem<TodoDetailContext>> = listOf(
        item()
            .atSlot(0)
        ...
        item()
            .atSlot(13)
            .displayAs {
                val todo = transaction { TodoManager.findById(context.todoId) }
                val tags = TagManager.tagLabelsForTodo(context.todoId)
                val lore = buildList<TextComponent> {
                    todo?.description?.takeIf { it.isNotBlank() }?.let { add(mm("<gray>\"$it\"")) }
                    if (tags.isNotEmpty()) add(mm("<gray>Tags: <white>${tags.joinToString(", ")}"))
                }
                createItem(Material.WRITABLE_BOOK, mm("<white>${todo?.name ?: "Unknown"}"), lore)
            },
