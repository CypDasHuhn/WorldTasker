...
object TodoManager {
    ...
    fun create(name: String, player: Player, description: String, location: Location?,): Int =
        transaction {
    ...
    fun findById(id: Int): TodoEntry? =
        transaction {
            Todos
                ...
                .firstOrNull()
                .let { if (it == null) null else TodoEntry.wrapRow(it) }
        }
    ...
    fun complete(id: Int, player: Player) = HistoryManager.record(id, player, TodoStatus.COMPLETE)

    fun reactivate(id: Int, player: Player) = HistoryManager.record(id, player, TodoStatus.REACTIVATE)
