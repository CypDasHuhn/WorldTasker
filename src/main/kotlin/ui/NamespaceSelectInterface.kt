...
data class NamespaceData(val id: Int, val name: String)

class NamespaceSelectContext(val filter: TodoFilter) : ScrollContext()
...
object NamespaceSelectInterface : ScrollInterface<NamespaceSelectContext, NamespaceData>(
    ...
    ScrollInterfaceOptions<NamespaceSelectContext>().apply {
        ...
        inventorySize = 9 * 4
        contentArea = (0 to 0) to (7 to 3)
    }
) {
    override fun contentProvider(id: Int, context: NamespaceSelectContext): NamespaceData? {
        return NamespaceManager.all().getOrNull(id)?.let {
            NamespaceData(it[NamespaceManager.Namespaces.id].value, it[NamespaceManager.Namespaces.name])
        }
    }

    override fun contentDisplay(data: NamespaceData, context: NamespaceSelectContext): InterfaceInfo<NamespaceSelectContext>.() -> ItemStack = {
        val tagCount = TagManager.byNamespace(data.id).size
        val activeCount = context.filter.included.count { tagId ->
    ...
    override fun contentClick(data: NamespaceData, context: NamespaceSelectContext): ClickInfo<NamespaceSelectContext>.() -> Unit = {
        TagSelectInterface.openInventory(click.player, TagSelectContext(data.id, context.filter))
    }

    override fun getOtherItems(): List<InterfaceItem<NamespaceSelectContext>> = listOf(
        item()
            .atSlot(8)
            .displayAs(createItem(
                Material.FEATHER,
                mm("<white>Back"),
